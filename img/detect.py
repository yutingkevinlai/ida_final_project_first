...
ans = np.where(np.logical_and(center>35, center<80))
ans = np.asarray(ans)
idx = ans[0]
print idx
...
# calculate intensity diff around nearest neighbors
	...
	for i in range(len(d[0])):
		for j in range(len(d[0])):
			if((d[i,j]<thresh)and(group[0,i]>group[0,j])):
				...
				# np.delete usage: np.delete(item, index, axis(row=0,col=1))				
...
constant = cv2.copyMakeBorder(test_img,30,30,30,30,cv2.BORDER_CONSTANT,value=[0,0,0])

print test_img.shape
for i in range(len(mean[0])):
	y = int(round(mean[0,i]))+30
	x = int(round(mean[1,i]))+30
	test_img[y,x] = 255
